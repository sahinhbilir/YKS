...
(async()=>{
 ...
 // Personal daily goals decorate the plan, never the FSRS history or issued slot order.
 init('2026-09-07'); await draw();
 const dailyHb=run('buHafta()'), originalPlan=layout(getPlan(dailyHb)), originalData=run('JSON.stringify([D.kart,D.log,D.elle])');
 check('daily-routine-three-simple-options',run('gorunumPlan()').includes('Karışık Paragraf')&&run('GUNLUK_EK_SECENEKLER.length')===3);
 check('manual-add-under-more',run('gorunumPlan()').includes('<summary>Daha fazla</summary>'));
 nodes.ogrGunlukSoru_paragraf=Object.assign(element(),{value:'25'});
 const dailyToggle=Object.assign(element(),{id:'',checked:true,dataset:{tur:'paragraf'},classList:{contains(n){return n==='ogrGunlukSec';}}});
 for(const fn of listeners.change||[])await fn({target:dailyToggle});
 run('gunlukEkAyarla("problem",15);gunlukEkAyarla("geometri",10);');await run('kaydet(true)');
 check('daily-selection-uses-question-input',run('gunlukEkSorusu(0,"paragraf",buHafta())')===25);
 check('daily-targets-do-not-change-fsrs-or-snapshots',originalData===run('JSON.stringify([D.kart,D.log,D.elle])')&&originalPlan===layout(getPlan(dailyHb)));
 check('daily-targets-repeat-seven-days',run('gunlukEkliPlan(0,buHafta(),planHesapla(0,buHafta())).gunler.every(l=>l.filter(x=>x.gunlukEk).length===3)'));
 check('daily-targets-continue-next-week',run('gunlukEkliPlan(0,buHafta()+7,planHesapla(0,buHafta()+7)).gunler[0].find(x=>x.ad==="Karışık Paragraf").soru')===25);
 check('daily-targets-appear-on-mobile-print-csv',run('planListesi(0,buHafta(),planHesapla(0,buHafta()),true)').includes('Karışık Problem')&&run('yazdirSayfa(0,buHafta())').includes('Karışık Geometri')&&run('csvOlustur(0,buHafta())').includes('Karışık Paragraf'));
 check('daily-decoration-does-not-duplicate-rows',run('(()=>{const p=gunlukEkliPlan(0,buHafta(),planHesapla(0,buHafta()));return gunlukEkliPlan(0,buHafta(),p).gunler[0].filter(x=>x.gunlukEk).length===3;})()'));
 run('D.ayar.testTarih="2026-09-09";gunlukEkAyarla("paragraf",30);gunlukEkAyarla("problem",0);');
 check('daily-edits-preserve-earlier-days',run('gunlukEkSorusu(0,"paragraf",buHafta())')===25&&run('gunlukEkSorusu(0,"paragraf",bugunNo())')===30);
 check('daily-disable-affects-only-today-onwards',run('gunlukEkSorusu(0,"problem",buHafta())')===15&&run('gunlukEkSorusu(0,"problem",bugunNo())')===0);
 await run('kaydet(true)');
 check('daily-settings-survive-backup-reload',run('(()=>{const copy=JSON.parse(JSON.stringify(D));veriyiHazirla(copy);return gunlukEkSorusu(0,"paragraf",bugunNo())===30;})()'));
 check('daily-question-validation',!run('gunlukEkAyarla("geometri",1.5)')&&!run('gunlukEkAyarla("geometri",201)'));
 run('EK.hafta=gunNo(D.ayar.sinav);');
 check('daily-goals-stop-at-exam',!run('gunlukEkAyarla("paragraf",20)')&&run('gunlukEkliPlan(0,haftaBasi(),planHesapla(0,haftaBasi())).gunler.flat().filter(x=>x.gunlukEk).length')===0);
 init('2026-09-07'); await draw();
 const protectedLayout=layout(getPlan(run('buHafta()')));
 await click('ogrManuelEkle');
 check('manual-student-modal-hides-audience',!!run('PENCERE.ogrenciEkle')&&!sandbox.document.body.lastHTML.includes('>Kime<'));
 run('PENCERE.gunSecim=[0,2];PENCERE.kapsam="hepsi"');
 check('manual-student-scope-cannot-expand',run('JSON.stringify(hedefOgrenciler())')==='[0]');
 nodes.pSerbestAd=Object.assign(element(),{value:'Kendi çalışma <notu>'});nodes.pSerbestDers=Object.assign(element(),{value:'Türkçe'});
 nodes.pSerbestSoru=Object.assign(element(),{value:'17'});
 await click('pSerbestEkle');
 check('manual-add-works-on-issued-week',getPlan(run('buHafta()')).gunler.flat().filter(x=>x.ad==='Kendi çalışma <notu>').length===2);
 check('manual-add-keeps-existing-rows-and-questions',layout({gunler:getPlan(run('buHafta()')).gunler.map(l=>l.filter(x=>x.ad!=='Kendi çalışma <notu>'))})===protectedLayout);
 check('manual-text-is-escaped',run('gorunumPlan()').includes('Kendi çalışma &lt;notu&gt;'));
 check('manual-free-work-does-not-fabricate-results',run('D.log.length===0&&Object.keys(D.kart).length===0'));
 const manualSlot=run('Object.keys(elleAl(0,buHafta()).ogrEk)[0]');sandbox.manualSlot=manualSlot;
 check('student-can-remove-own-addition',run('ogrenciElleKaldir(manualSlot)'));
 check('student-cannot-remove-scheduled-work-through-extra-controls',!run('ogrenciElleKaldir(Object.keys(elleAl(0,buHafta()).yer).find(k=>!k.startsWith("OGREK")))'));
 run('PENCERE={ogrenciEkle:true};ogrenciElleEkle(0,buHafta(),[1],{tur:"konu",ki:0,ad:konuAl(0)[3],ders:konuDersAdi(0),soru:12});');
 check('manual-topic-has-result-entry',run('gorunumGiris()').includes('data-ki="0"'));
 run('D.ayar.testTarih="2026-09-08";sonucIsle(0,[{ki:0,gun:bugunNo(),dogru:10,soru:12}]);');
 check('manual-topic-enters-fsrs-after-real-result',run('D.kart["0:0"].n')===1);
 const scoredSlot=run('Object.keys(elleAl(0,buHafta()).konuSlot).find(k=>elleAl(0,buHafta()).konuSlot[k].ki===0)');sandbox.scoredSlot=scoredSlot;
 check('scored-extra-cannot-be-removed',!run('ogrenciElleKaldir(scoredSlot)'));
 check('manual-add-rejects-past-days',run('ogrenciElleEkle(0,buHafta(),[0],{tur:"serbest",ad:"Past",soru:10})')===0);
 run('EK.hafta=buHafta()+7;PENCERE={ogrenciEkle:true};ogrenciElleEkle(0,haftaBasi(),[0],{tur:"serbest",ad:"Next-week addition",soru:15});');await run('kaydet(true)');
